...
public class PathObjectHandler implements Loggable {
    ...
    private PathObjectHandler(){
        sortedOptions = new TreeSet<>(
		        Arrays.asList("Enter", "Cross", "Pass", "Open", "Close", "Walk-through", "Use", "Pass" +
                                "-through",
                        "Exit",
    ...
    private enum SpecialObject {
        ...
        SOUL_WARS_INSIDE("Portal", "Edgeville", new RSTile(2205,2857,0), new SpecialCondition() {
            @Override
            boolean isSpecialLocation(PathAnalyzer.DestinationDetails destinationDetails) {
                return destinationDetails.getDestination().getRSTile().equals(new RSTile(3081,3475,0));
            }
        }),
        ...
        if (specialObject != null) {
            ...
            switch (specialObject){
                ...
                case BRINE_RAT_CAVE_BOULDER:
                    ...
                    break;
                case SOUL_WARS_INSIDE: {
                    if(!clickOnObject(object,specialObject.getAction())){
                        return false;
                    }
                    successfulClick = true;
                    WaitFor.condition(10000, () ->
                            SpecialObject.SOUL_WARS_INSIDE.getLocation().equals(Player.getPosition()) ?
                                    WaitFor.Return.SUCCESS : WaitFor.Return.IGNORE);
                }
                break;
            }
    ...
    private static boolean clickOnObject(RSObject object, String... options){
        ...
        } else if (isSoulWarsPortalInside(object)) { // Polymorphic on TRiBot
            result = handleSoulWarsPortalInside(object);
        } else {
    ...
    /**
     * TRiBot
     * Author Polymorphic (Jackson) March 12, 2021, 9:26 AM
     * @param object
     * @return
     */
    private static boolean isSoulWarsPortalInside(RSObject object) {
        List<String> portalName = Collections.singletonList("Portal");
        return portalName.contains(object.getDefinition().getName()) && object.getPosition().equals(new RSTile(2205,
                2857,0)); // the portal position.
    }
     ...
     * @return
     */
    private static boolean handleSoulWarsPortalInside(RSObject object) {
        if (getActions(object).contains("Edgeville")) {
            getInstance().log("Interacting with (" + object.getDefinition().getName() + ") at " + object.getPosition() + " with option: Edgeville");
            return InteractionHelper.click(object, "Edgeville");
        }
        return false;
    }
