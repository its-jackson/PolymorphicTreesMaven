...
public final class AntiBan {
    ...
    public static boolean afkMicroSleep() {
        if (micro_sleep) {
            incrementABCCount();
            long timerRun = Timing.timeFromMark(getAbcStartTime());
            final long magic = General.randomLong(600000, 650000); // 10-15 minutes
            final long nine_minutes = 450000;
            //final long magic_debug = 1000;
            // old - 35
            final int sleep_magic = 40 * getReactionTime();
            if (timerRun > magic) {
                if (!(sleep_magic >= nine_minutes)) {
